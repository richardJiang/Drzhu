	...
	if ( strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger')) {
        ...
        $data = curl_exec($ch);
			...
			if(!empty($xml->err_code)&&$xml->err_code=='OUT_TRADE_NO_USED')
			{
						...
						$randomorder = mysqldselect("SELECT id FROM " . table('shop_order') . " WHERE  ordersn=:ordersn   limit 1", array(':ordersn' =>$ordersns));
          	while(!empty($randomorder['id']))
          	{
          				$ordersns= date('YmdHis') . random(6, 1);
          			$randomorder = mysqld_select("SELECT id FROM " . table('shop_order') . " WHERE  ordersn=:ordersn   limit 1", array(':ordersn' =>$ordersns));
          
          	}
          	
				mysqld_update('shop_order',array('oldordersn'=>$order['ordersn'],'ordersn'=>$ordersns),array('id'=>$order['id']));
				
					header("Location:".WEBSITE_ROOT.create_url("mobile",array("name"=>"modules","do"=>"bridgepay","osn"=>urlencode(base64_encode($ordersns)), "gtitle"=>'')));
	exit;
...
<!-- order 订单信息 end -->            <!-- order 订单信息 end -->
            ...
            <div class="payment">
                ...
                <div class="payment-change">
                    <a class="pc-wrap" id="reChooseUrl" href="javascript:history.back();">
                       ...
                       <a class="pc-wrap" style="float:right" href="<?php echo WEBSITE_ROOT.create_url('mobile',array('name' => 'shopwap','do' => 'myorder'));?>">
  ...
  <script type="text/javascript" language="javascript">
      function checkorder()
      {
      	$.getJSON("<?php echo 	create_url('mobile',array('name' => 'shopwap','do' => 'getorder','id'=>$order['id']));?>", { }, function(json){

  if(json.status>0)
  {
  location.href="<?php echo WEBSITE_ROOT.create_url('mobile',array('name' => 'shopwap','do' => 'myorder'));?>";	
  }
});
      }
