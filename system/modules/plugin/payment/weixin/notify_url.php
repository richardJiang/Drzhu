	...
	ksort($array_data, SORT_STRING);
      ...
      $time_end=$array_data['time_end'];
	      	...
	      	mysqld_insert('paylog_weixin', $paylog_weixin);
						...
						mysqld_insert('paylog', array('typename'=>'支付成功','pdate'=>$xml,'ptype'=>'successes','paytype'=>'weixin',"beid"=>$_CMS['beid']));
                            mysqld_insert('paylog', array('typename'=>'下面是返佣','pdate'=>$order["openid"]."---".$orderid,'ptype'=>'successed','paytype'=>'weixin',"beid"=>998));
                            //发放佣金奖励 jackdou
                            require_once WEB_ROOT.'/system/modules/plugin/payment/successDispatch.php';
                            $dispatch = new successDispatch('weixin',$_CMS);
                            $dispatch->dispatchMoney($order["openid"],$orderid);
                            unset($dispatch);
                            //结束
