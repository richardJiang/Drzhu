 ...
 $operation = !empty($_GP['op']) ? $_GP['op'] : 'display';
		 ...
		 if ($operation == 'display') {
           ...
           if ($status != -99) {
			...
			$totalprice =   mysqld_selectall("SELECT *,sum(price) as totalprice FROM " . table('shop_order') . " WHERE 1=1 $condition and beid=:beid ORDER BY  createtime DESC ".$selectCondition,array(':beid'=>$_CMS['beid']));
