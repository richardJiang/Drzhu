               ...
               $piclist = mysqld_selectall("SELECT * FROM " . table('shop_goods_piclist') . " where goodid=$id and beid=:beid ORDER BY id ASC",array(':beid'=>$_CMS['beid']));
                ...
                if (count($options) > 0) {
                    ...
                    for ($i = 0; $i < $newlen; $i++) {
                        ...
                        $val = array("id" => "","title"=>"", "stock" => "", "costprice" => "", "productprice" => "", "marketprice" => "", "weight" => "", "ffee" => "", "pfee" => "", "mfee" => "");
                        if(!empty($options))
                        {
            ...
            if (checksubmit('submit')) {
                ...
                $data = array(
                    ...
                    'timeend' => strtotime($_GP['timeend']),
					 'ffee' => intval($_GP['ffee']),
					  'pfee' => intval($_GP['pfee']),
					   'mfee' => intval($_GP['mfee'])
                    );
