...
function member_login_weixin($weixin_openid,$invitation_code="")
{
    ...
    if (!empty($weixin_openid)) {
        ...
        if (!empty($weixin_wxfans['weixin_openid'])) {
            ...
            if (!empty($member['openid'])) {
                ...
                $openid = member_create_new("", "", $weixin_wxfans['nickname'],$invitation_code);
 ...
 * @return string
 */
function member_create_new($mobile, $pwd, $nickname = "", $invitation_code = '')
{
    ...
    $data = array(
        ...
        'openid' => $openid,
        'beid' => $_CMS['beid'],
        'invitationCode' => $invitation_code, //邀请人用户编号jackdou
    );
    mysqld_insert('member', $data);
    return $openid;
}

function member_create_new_sample($nickname, $invitation_code = '')
{
    global $_CMS;
    $openid = date("YmdH", time()) . rand(100, 999) . rand(100, 999) . rand(100, 999) . rand(0, 9);
    $hasmember = mysqld_select("SELECT * FROM " . table('member') . " WHERE openid = :openid  ", array(':openid' => $openid));
