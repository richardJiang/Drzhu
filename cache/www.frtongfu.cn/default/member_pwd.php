...
<div id="container"></div>
    ...
    <div class="info_main">
       	...
       	<?php if(!empty($memberinfo['mobile'])){?><div class="line"><div class="title">手机号</div><div class='info'><div class='inner'><?php echo $memberinfo['mobile'];?></div></div></div><?php }?>
        <div class="line"><div class="title">原密码</div><div class='info'><div class='inner'><input type="password" id='oldpwd' name='oldpwd' placeholder="请输入原密码"  value="" /></div></div></div>
        <div class="line"><div class="title">新密码</div><div class='info'><div class='inner'><input type="password" id='pwd' name='pwd' placeholder="请输入新密码"  value="" /></div></div></div>
    </div>
    <div class="info_sub" onclick="submitRegister();" id="submitRegister">确认修改</div>
...
var password = $('input[name=oldpwd]').val().trim();
	if (!/^.{6,16}/.test(password)) {
		alert('请正确输入原始密码，不能少于6位');
		return false;
	}
	var password = $('input[name=pwd]').val().trim();
	if (!/^.{6,16}/.test(password)) {
		alert('请正确设置新密码，不能少于6位');
		return false;
	}
		$('#submit').click();
