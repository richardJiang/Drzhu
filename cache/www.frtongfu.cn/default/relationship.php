...
<html><head>
    ...
    <link rel="stylesheet" type="text/css" href="http://www.frtongfu.cn/themes/default/__RESOURCE__/recouse/css/fd.css">
    <link rel="stylesheet" type="text/css" href="http://www.frtongfu.cn/themes/default/__RESOURCE__/recouse/css/fcommom.css">
<body>
...
<div id="container">
    ...
    <div class="cart_top" style="background: #008cd7;">
        <div class="title" onclick="history.back()" style="color: #fff;text-align:center;"><span style="float:left"><i class="fa fa-chevron-left" ></i>&nbsp;</span>关系树</div>
    </div><div class="qrcode" style="padding:10px;text-align:center;color:blue;">
	<a href='/index.php?mod=mobile&op=history2&name=shopwap&do=outchargegold&beid=1' style="color:#9c9191;">
    
        余额明细表
    </a></div>
    <div class="qrcode">
        <div class="user-name">会员：<?php  echo empty($member['nickname'])?$member['mobile']:$member['nickname'];?>	的关系树</div>
    </div>
	<div class="cart">
		<div>下一层会员：</div><ul><?php  foreach($membersfirst as $v){
		    $name = $v['realname']?$v['realname']:'暂无姓名';
			$mobile = $v['mobile']?$v['mobile']:'暂无手机';
			$money = $v['money']?$v['money']:'暂无业绩';
			echo "<li><div>姓名:  $name   手机号:  $mobile   业绩:  $money</div></li>";
		};?></ul>
	</div>
	<div class="cart">
		<div>下二层会员：</div><ul><?php  foreach($memberSec as $v){
		    $name = $v['realname']?$v['realname']:'暂无姓名';
			$mobile = $v['mobile']?$v['mobile']:'暂无手机';
			$money = $v['money']?$v['money']:'暂无业绩';
			echo "<li><div>姓名:  $name   手机号:  $mobile   业绩:  $money</div></li>";
	...
	<div class="cart">
		<div>下三层会员：</div><ul><?php  foreach($memberThr as $v){
		    $name = $v['realname']?$v['realname']:'暂无姓名';
			$mobile = $v['mobile']?$v['mobile']:'暂无手机';
			$money = $v['money']?$v['money']:'暂无业绩';
			echo "<li><div>姓名:  $name   手机号:  $mobile   业绩:  $money</div></li>";
