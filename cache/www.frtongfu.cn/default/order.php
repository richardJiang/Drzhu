...
<?php  if((!empty($item['is_system'])||!empty($item['is_be']))&&(empty($item['status'])||$item['status']==-1)) {$nodate=false; ?>
		...
		<?php  if($item['status']==0) { ?>
		    <div class="sub1"  onclick="location.href='<?php  echo mobile_url('pay', array('orderid' => $item['id'], 'topay' => '1'))?>';">订单付款</div>
                   ...
                   <div class="sub2 order_delete"  onclick="$.confirm({title: '',content: '确定此关闭订单吗？',  confirm: function(){location.href='<?php  echo create_url('mobile',array('name' => 'shopwap','do' => 'myorder','op'=>'orderclose','orderid' => $item['id']))?>';}});">关闭订单</div>
 <?php  }?>
        ...
        <div class="sub">
        	  <div class="sub1" style="border: 1px solid green;background: green;"  onclick="location.href='<?php  echo create_url('mobile',array('name' => 'shopwap','do' => 'myorder','op'=>'detail','orderid' => $item['id'],'is_system'=>1))?>';">订单详情</div>
                  ...
                  <div class="sub1 order_complate"  onclick="$.confirm({title: '',content: '确认收货？',  confirm: function(){location.href='<?php  echo create_url('mobile',array('name' => 'shopwap','do' => 'myorder','op'=>'zong_confirm','orderid' => $item['id']))?>';}});">确认收货</div>
	<?php  }  ?>
     ...
     <div class="sub1" style="border: 1px solid green;background: green;" onclick="location.href='<?php  echo create_url('mobile',array('name' => 'bj_hx','do' => 'sendweixin','orderid' => $item['id']))?>'">发送到微信</div>
        ...
        <div class="sub">
        	      ...
        	      <div class="sub1 order_complate"  onclick="$.confirm({title: '',content: '确认收货？',  confirm: function(){location.href='<?php  echo create_url('mobile',array('name' => 'shopwap','do' => 'myorder','op'=>'be_confirm','orderid' => $item['id']))?>';}})">确认收货</div>
